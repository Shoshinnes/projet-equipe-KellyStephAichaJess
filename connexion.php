...
//-------------------------TRAITEMENT----------------------------
    ...
    //demande de deconnexion par l internaut
    if(isset($_GET['action']) && $_GET['action'] == 'deconnexion'){
        session_destroy(); //suppression de la session
    }

    //internaut deja connecté
    if(internautEstConnecte()){
        //On le renvoie vers son profil
        header('location:moncompte.php');
    }
    
    //traitement du formulaire de connection et remplissage de la session
    if($_POST){
        
        $resultat = executeRequete("SELECT * FROM membres WHERE confirmemail = '$_POST[confirmemail]'");
        
            if($resultat->num_rows != 0){ //num_rows donne le nombre d elements, de lignes etc
                
                $membres = $resultat->fetch_assoc();
                
                //verification du mdp
               /* if($membres['mdp'] == md5($_POST['mdp'])){ //probleme avec le md5*/
                if($membres['mdp'] == ($_POST['mdp'])){    
                    foreach($membres as $indice=>$valeur){
                        $_SESSION['membres'][$indice] = $valeur;
                    }
                    header('location:moncompte.php'); // Equivaut au else{}
                ...
                $contenu .='<div class="erreur">Erreur sur le pseudo.</div>';
            }   
            //Fin du if($resultat->num_rows != 0)
...
<div class="form-group label-floating  ">
    <label class="control-label required" for="confirmemail">Votre email</label>
    <input class="firstname form-control" type="text" name="confirmemail" id="confirmemail" required>
</div>
</div>
</div>
...
<div class="col-sm-6">
<div class="form-group label-floating  ">
    <label class="control-label required" for="mdp">Mot de passe</label>
    <input class="firstname form-control" type="password" name="mdp" id="mdp" required>
  </div>
</div>
</div>
    
    
    <input class="btn btn-primary" type="submit" value="se connecter" name="seconnecter">
